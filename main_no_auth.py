...
# --- Cross-platform compatibility shims (Windows/Linux) ---
import builtins as _builtins
import glob

def _normalize_path_arg(_arg):
    try:
        if isinstance(_arg, str):
            return _arg.replace("\\", os.sep)
    except Exception:
        pass
    return _arg

# Normalize paths for any direct string paths passed to open()
_original_open = _builtins.open
def _open_wrapper(_file, *args, **kwargs):
    return _original_open(_normalize_path_arg(_file), *args, **kwargs)
_builtins.open = _open_wrapper

# Normalize common os.* path consumers used by this script
_original_mkdir = os.mkdir
def _mkdir_wrapper(_path, *args, **kwargs):
    try:
        return _original_mkdir(_normalize_path_arg(_path), *args, **kwargs)
    except FileExistsError:
        return None
os.mkdir = _mkdir_wrapper

_original_listdir = os.listdir
def _listdir_wrapper(_path):
    return _original_listdir(_normalize_path_arg(_path))
os.listdir = _listdir_wrapper

_original_chdir = os.chdir
def _chdir_wrapper(_path):
    return _original_chdir(_normalize_path_arg(_path))
os.chdir = _chdir_wrapper

# Map Windows 'cls' to Linux 'clear'
_original_system = os.system
def _system_wrapper(cmd):
    if isinstance(cmd, str) and cmd.strip() == "cls" and os.name != "nt":
        return _original_system("clear")
    return _original_system(cmd)
os.system = _system_wrapper

# Stub SetConsoleTitleW on non-Windows
if os.name != "nt":
    try:
        class _DummyKernel32:
            def SetConsoleTitleW(self, *args, **kwargs):
                return None

        class _DummyWindll:
            kernel32 = _DummyKernel32()

        ctypes.windll = _DummyWindll()
    except Exception:
        pass

# --- Config loading (externalized lists) ---
...
def get_config_list(key, default_list):
    _cfg = load_config()
    _lst = _cfg.get(key)
...
# --- Local no-op fallbacks for removed GradientGen ---
def PrintGradient(_c1, _c2, _text):
    try:
        print(_text)
    except Exception:
        pass

def PrintLinearGradient(*_args, **_kwargs):
    return None
...
$$\   $$ |$$\   $$ |$$ |$$   ____|$$ |        $$ |$$\ $$ |\$$$ |$$ |      
\$$$$$$  |\$$$$$$  |$$ |\$$$$$$$\ \$$$$$$$\   \$$$$  |\$$$$$$  /$$ |      
 \______/  \______/ \__| \_______| \_______|   \____/  \______/ \__|                
\033[37m
"""
...
def getcombo(title="FILE"):
    # Prefer GUI picker if available
    if fd is not None:
        try:
    ...
    # Fallback: CLI prompt
    try:
        path = input(f"Enter path to {title} (.txt): ").strip()
        if path:
            return path
    except Exception:
        pass
...
def password_limit(limit):
    raise NotImplementedError("moved to modules.password_edits.password_limit")


@f1
def password_hex(limits):
    raise NotImplementedError("moved to modules.password_edits.password_hex")


@f1
def domain_changer(dom):
    raise NotImplementedError("moved to modules.email_edits.domain_changer")


@f1
def ComboLineCounter():
    raise NotImplementedError("moved to modules.combo_edits.ComboLineCounter")


@f1
def reverser():
    raise NotImplementedError("moved to modules.combo_edits.reverser")


@f1
def combo_sorter():
    raise NotImplementedError("moved to modules.combo_edits.combo_sorter")


@f1
def email_extracor():
    raise NotImplementedError("moved to modules.email_edits.email_extracor")


@f1
def password_extracor():
    raise NotImplementedError("moved to modules.email_edits.password_extracor")


@f1
def shuffle():
    raise NotImplementedError("moved to modules.combo_edits.shuffle")


@f1
def remove_dupes():
    raise NotImplementedError("moved to modules.combo_edits.remove_dupes")


@f1
def CLEANER():
    raise NotImplementedError("moved to modules.combo_edits.CLEANER")


@f1
def filter_combo_by_domain(domain):
    raise NotImplementedError("moved to modules.email_edits.filter_combo_by_domain")


@f1
def LQTOHQ():
    raise NotImplementedError("moved to modules.combo_edits.LQTOHQ")
    ...
    def Keywords_Cleaner(file=None):
        def clean(file):
            ...
            for line in input_file:
                ...
                line = re.sub(f"[{re.escape(blacklist_chars)}]", "", line)
...
def Filter_urls():
    ...
    bad_urls = get_config_list("bad_urls", [
        "https://bing",
        "https://wikipedia",
        "https://stackoverflow",
        "https://amazon",
...
def get_combos():
    ...
    while pagenum < num:  # simple loop to scrape page source
        req = requests.get(f"https://COMBOCLOUD.REPLACETHIS/t/combolists?page={pagenum}")
        src = req.text
...
def dorkbeg():
    ...
    if pagtyes == None:
        pagtypes = get_config_list("pagtypes", [])
    else:
        pagtypes = []
...
# Override password edit functions with modular versions
try:
    from modules.password_edits import (
        password_limit,
        password_hex,
        lowercase_pass,
        upper_password,
        add_prefix_to_password,
        add_suffix_to_password,
    )
    from modules.email_edits import (
        domain_changer,
        email_extracor,
        password_extracor,
        filter_combo_by_domain,
        EP_TO_UP,
        domain_sorter,
    )
    from modules.combo_edits import (
        CLEANER,
        reverser,
        combo_sorter,
        shuffle,
